...
RRSet *
RRSet_GLB_RR::add_answers(NTD *ntd, int qkl, int qty,int flag) const {
    float totwgt = 0;
    RRSet *best = 0;
...
RRSet_GLB_MM::add_answers(NTD *ntd, int qkl, int qty,int flag) const {
    ...
    if( MMDB::locate(ntd) ){
        ...
        RRSet * rs = a_a_failover_specify(":unknown", ntd, qty,flag);
        if( rs ) return rs;
